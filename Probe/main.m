...
- (BOOL)application:(UIApplication *)application
  ...
  // No button for the code-loading test: enabling JIT relaunches the app with
  // CS_DEBUGGED already set, so the ordinary launch-time run observes it. The
  // logs bear that out -- the automatic section F reports CS_DEBUGGED=1 on a
  // JIT-enabled launch and 0 otherwise, which is exactly the comparison
  // wanted, without a second trigger.
  //
